        ...
        <b-table id="db-users-table" 
            ...
            striped bordered outlined small hover dark>
                <template #cell(add)="row" >
                    <b-form-checkbox :value="`${row.item.userId}`" size="sm" class="mr-2"
                    variant="info" @change="toggleAll"></b-form-checkbox>
                </template>
        </b-table>
    </b-row>
    <b-row class="float-right m-2">
        <b-button variant="info" class="m-1" size="sm" @click="addUsersToCurrentGroup()">Add</b-button>
        <b-button variant="info" class="m-1" size="sm">Cancle</b-button>
    </b-row>
    <b-row>
            <b-col md="5"></b-col>
...
export default Vue.extend({
    ...
    data () {
        return {
            fields: ['userId', 'userName', 'emailId', 'contactNo', 'country', 'isActive', 'Add'],
            items: [],
            usersAll: [],
            addUsersToGroupArr :{
                userIDsArr: [],
    ...
    methods: {
        async fetchAllUsersFromDB(){
           await this.$axios.$get(`${process.env.BASE_URL}/users/userall`).then((res) => {
        ...
        toggleAll(checked: any) {
            ...
            console.log('userIDsArr=',checked , this.addUsersToGroupArr.userIDsArr.length);
            console.log('userIDsArr=', this.addUsersToGroupArr.userIDsArr);
        },
        
        async addUsersToCurrentGroup(){
            this.addUsersToGroupArr.groupID = this.groupInfos.groupId ;
            console.log(this.addUsersToGroupArr);

            await this.$axios.$post(`${process.env.BASE_URL}/group/addusers`, (this as any).addUsersToGroupArr)
            .then((res) => {
                console.log(res);
                 (this as any).toastMessage('success', 'Added user(s) has been successful..'); 
                })
            .catch((error) => {
                console.log(error);
                (this as any).toastMessage('danger', 'Failed to add user(s) to group');
            });
        },
