    ...
    <b-row class="mt-1 pt-3">
      ...
      <b-col md="6">
        <b-card style="background-color: #f7f7f7">
          ...
          <b-span
            ><h3>{{ profile.userName }}</h3></b-span
          >
          <h6>User ID: {{ $store.state.auth.user.userId }}</h6>
          <b-card-text>
            A super admin user can perform all opertions in this product. And
            has ability to change this appplication.
          </b-card-text>

          <b-card-text
            >All information about this user are mentioned below step by
            step.</b-card-text
          >

          <h6>
            Password: <b-span>{{ profile.password }} </b-span>
          </h6>
          ...
          <b-button
            ...
            class="mr-2"
            variant="info"
            @click="setUserId($store.state.auth.user.userId)"
            >Edit</b-button
          >
...
export default Vue.extend({
  ...
  data() {
    return {
      ...
      updatedAt: new Date(
        this.$store.state.auth.user.updatedAt
      ).toLocaleString(),
      delatedAt: new Date(
        this.$store.state.auth.user.delatedAt
      ).toLocaleString(),
    }
  },
  computed: {
  ...
  methods: {
    async fetchAuthUserInfo() {
      console.log(this.$store.state.auth.user.userId)
      await this.$store
        .dispatch('user/fetchUser', this.$store.state.auth.user.userId)
        .then(() => {
          ;(this as any).profile = { ...(this as any).clickedUser }
          console.log('call profile:: ', (this as any).profile)
        })
